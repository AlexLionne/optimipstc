        ...
        return ( <div style={{width:width,height:height,overflowX: 'auto'}}>
                                        ...
                                        if(source.data.common_child !== null){
                                            ...
                                            sources.map((sub_source,j)=>{
                                                if(sub_source.data.id === child_id){
                                                    if(!containsAdvancedObject({source:source,target:sub_source},final_sources)){
            ...
            <CardAnim pose={this.state.card_isopen ? 'visible' : 'hidden'} >
                <Card style={styles.card_detail}>
                    <CardBody>
                        <CardTitle style={{width:'auto',fontWeight:'bold', color:'#000',display:'inline-block'}}>{this.state.node !== null ? this.state.node.data.name : null}</CardTitle>
                        <FontAwesomeIcon  color={"#000"} style={{float:'right',display:'inline-block'}} onClick={this.closeCard} icon={faTimes} />
                        <CardText>
